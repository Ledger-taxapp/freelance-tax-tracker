...
- **PWA files** — `manifest.json` / `demo-manifest.json` (name, icons, colors, standalone display — one per entry point so each can be installed to a home screen independently) and `sw.js` (a service worker doing stale-while-revalidate caching for offline support), with matching icon assets in `icons/`. Both `index.html` and `demo.html` register `sw.js` and link their respective manifest in `<head>`.
