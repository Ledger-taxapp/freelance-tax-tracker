...
A freelance/self-employed federal tax estimator: log income, expenses, mileage, and estimated payments, and it projects SE tax, QBI deduction, Additional Medicare Tax, federal income tax, and a safe-harbor quarterly payment target. Each build is one **self-contained HTML file** — inline `<style>` and inline `<script>`, no build step, no bundler, no external JS dependencies (only Google Fonts are loaded remotely). Not a git repo. It's grown past "single-page" — Ledger, Dashboard, and Terms are already separate views within the file (see Architecture below), and more views/pages may be added later.

There is no test suite, linter, or package.json. "Running" the app means opening the HTML file directly (double-click, or `open freelancer-tax-tracker-vX.Y.html`). Validate changes by loading the file in a browser and exercising the flow by hand — add an entry, switch tax years, switch filing status, check Dashboard/Terms views.

## File/versioning convention

- Files are named `freelancer-tax-tracker-vX.Y.html`, one file per shipped version. **Never overwrite an old version in place** — copy the latest version forward to a new `vX.Y` filename before editing, matching the existing pattern.
- `freelancer-tax-tracker-vX.Y-demo.html` is the same app pre-loaded with sample data (a fictional landscaping business, ~165 entries) for demoing; it should stay in sync feature-for-feature with the corresponding non-demo version.
- [tax-tracker-changelog_1.md](tax-tracker-changelog_1.md) is the running build log. **Append new version entries at the bottom; never edit past entries.** Follow the existing entry format (`## vX.Y — short title` + bullet list of changes). The current latest version and the "deferred / discussed but not yet built" list at the bottom of the changelog are the best source of what's next.

## Architecture (per file)

Everything lives in one IIFE at the bottom of the file. Key pieces, in `freelancer-tax-tracker-v1.15.html` line order:
...
- **`state`** — the single mutable app state object (filing status, entries array, W-2/withholding/prior-year fields, selected year). Persisted via `window.storage.get/set` (a host-provided async storage API — **not** `localStorage`), keyed by `STORAGE_KEY`. `save()` fires on every mutation; `load()` runs once at startup and calls `renderAll()` when done.
